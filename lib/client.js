...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 某时刻的北京日历日（"YYYY-MM-DD"），用来核对法定节假日。 */
    ...
    /** 该时刻是否落在服务端下发的「法定节假日」名单里（服务端没下发就当不是）。 */
    function isHolidayDate(date, holidays) {
      if (!Array.isArray(holidays) || holidays.length === 0) return false;
      var key = beijingDateKey(date);
      return key !== "" && holidays.indexOf(key) >= 0;
    }

    /**
     * 是否处于高峰时段。官方口径（2026-09-19《API 峰谷时间说明》）：周末（含调休上班的
     * 周末）与中国法定节假日全天都按空闲时段计费，只有普通工作日的 09-12 / 14-18 是高峰。
     * `holidays` 由服务端随价格表下发；拿不到时退化为"只认周末"，与旧版行为一致。
     */
    function isPeakNow(date, holidays) {
      var d = date || new Date();
      if (beijingWeekend(d)) return false;
      if (isHolidayDate(d, holidays)) return false;
      var h = beijingHour(d);
      return (h >= 9 && h < 12) || (h >= 14 && h < 18);
    }

    function estimateCostFromUsage(usage, rates, holidays) {
      if (!usage || typeof usage !== "object") return 0;
      var peak = isPeakNow(null, holidays);
    ...
    function PricingBody(props) {
      ...
      // 说清"今天为什么是闲时"：官方口径里周末、调休上班的周末、法定节假日都是全天闲时。
      var todayNote = "";
      if (nowInfo.holiday) todayNote = "（今天 " + nowInfo.holiday + " 假期，全天闲时）";
      else if (nowInfo.makeupWorkday) todayNote = "（今天调休上班，官方口径仍全天闲时）";
      else if (nowInfo.weekend) todayNote = "（今天周末，全天闲时）";
      rows.push(h("div", { key: "now-title", style: SECTION_TITLE },
        "现在生效的价 · " + (nowInfo.mode === "peak" ? "高峰时段" : "闲时") + todayNote + "（元/百万 token：输入 / 缓存命中 / 输出）"
      ));
      ...
      // 节假日表自检：表靠人补（官方没有机器可读接口）。服务端算好 stale/missingYear 与提示
      // 文案，这里只负责显示——判定与措辞都在 pricing.js，便于测试锁住，也免得前端抄一份日期表。
      var coverage = state.coverage;
      if (coverage !== null && coverage !== void 0 && coverage.stale === true && coverage.message !== "") {
        rows.push(h("div", { key: "coverage", style: { color: "var(--dsw-alias-state-error-primary)", fontSize: 11, lineHeight: "16px" } }, coverage.message));
      }
        ...
        rows.push(h("div", { key: "hist", style: { display: "flex", flexDirection: "column", gap: 6 } }, state.official.map(function (policy, index) {
          ...
          return h("div", { key: key, style: { borderLeft: "2px solid var(--dsw-alias-border-l1)", paddingLeft: 6 } },
            h("div", { style: { display: "flex", alignItems: "center", gap: 6, fontSize: 11, fontWeight: 600, lineHeight: "16px", flexWrap: "wrap" } },
              ...
              policy.holidayOffPeak ? h("span", { style: BADGE_STYLE }, "法定节假日全天闲时") : null,
      ...
      if (ui.openAdvanced) {
        rows.push(h("div", { key: "adv", style: { display: "flex", flexDirection: "column", gap: 4, border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 8, padding: 6 } },
          ...
          h("div", { style: MUTED },
            "官方口径（自动生效、不用设）：中国法定节假日与调休上班的周末全天按闲时价。"
              + (state.calendar && Array.isArray(state.calendar.periods)
                ? "2026 年：" + state.calendar.periods.map(function (period) {
                  return period.name + " " + period.from.slice(5) + "~" + period.to.slice(5);
                }).join("、")
                : "")
          ),
          state.calendar && Array.isArray(state.calendar.makeup) && state.calendar.makeup.length > 0
            ? h("div", { style: MUTED },
              "调休上班日（照旧按闲时价）：" + state.calendar.makeup.map(function (day) { return day.slice(5); }).join("、"))
            : null,
          h("div", { style: MUTED }, "下面的勾选项只作用于「我的调价记录」里填的价；没自定义的模型永远按官方历史价算。"),
    ...
    function ConversationCostButton(props) {
      ...
      var [panelAt, setPanelAt] = useState(null);
      ...
      var [holidays, setHolidays] = useState(null);
      ...
      if (sessionId !== void 0) {
        ...
        for (var li = 0; li < localIds.length; li++) {
          ...
          if (usage) {
            localCost += estimateCostFromUsage(usage, flashRates, holidays);
        ...
        var load = async function () {
          ...
          try {
            ...
            if (!cancelled && priceBody && priceBody.now && priceBody.now.rows) {
              ...
              // 法定节假日名单随价格表一起下发：本地估算（服务端还没结算时用）也得按官方
              // 口径把节假日算成闲时。旧版服务端没有这个字段 → 退化为"只认周末"。
              var holidayList = priceBody.calendar && Array.isArray(priceBody.calendar.holidays)
                ? priceBody.calendar.holidays
                : null;
      ...
      if (open) {
        ...
        panel = h("div", {
          style: {
            position: "absolute",
            // 贴窗口右侧显示（打开时量出来的坐标），不再跟着按钮的 x 走：会话标题短的时候按钮
            // 落在窗口中偏左，面板从按钮向左伸出去会被左侧会话区（层级更高）盖住左半边。
            top: panelAt === null ? "calc(100% + 8px)" : panelAt.top,
            right: panelAt === null ? 0 : panelAt.right,
            zIndex: 300,
            // 窄窗口自适应：固定 440px 在只有七八百 CSS 像素宽的窗口里会占掉半屏以上，把界面挡住。
            // 宽屏维持 440（观感不变），窄窗按 40vw 收窄，下限 320px 保内容可读。
            width: panelAt === null ? "clamp(320px, 40vw, 440px)" : panelAt.width,
            maxWidth: "calc(100vw - 32px)",
            overflowX: "hidden",
            maxHeight: panelAt === null ? "min(560px, calc(100vh - 140px))" : ("min(560px, " + panelAt.maxHeight + "px)"),
            overflowY: "auto",
            boxSizing: "border-box",
            border: "1px solid var(--dsw-alias-border-l2)",
      ...
      // 打开前量一次：面板仍然挂在「计费标签下方」，但宽度压到"左边不被会话区盖住"为止；
      // 实在不够时再整体往右挪一点点（不挪到窗口最右边，看着还像挂在标签下面）。
      // 依据＝从按钮往上找祖先里最靠左的那条边界（≥40px 才算，用来滤掉整宽外壳的 0/8/16），
      // 那基本就是对话区的左边缘；用几何量而不是类名，DSH 升级不会失效。
      var togglePanel = function () {
        var next = !open;
        if (next && rootRef.current !== null) {
          var box = rootRef.current.getBoundingClientRect();
          var bound = 8;
          for (var node = rootRef.current; node !== null && node !== document.body; node = node.parentElement) {
            var left = Math.round(node.getBoundingClientRect().left);
            if (left >= 40 && left < box.right - 24 && (bound === 8 || left < bound)) bound = left;
          }
          var room = Math.round(box.right - bound - 8);               // 往左最多能伸多少
          var width = Math.max(260, Math.min(440, room));             // 再窄内容就没法看了
          var maxShift = Math.max(0, Math.round(window.innerWidth - 16 - box.right));
          var shift = Math.min(Math.max(0, width - room), maxShift);  // 压不下去的部分靠右挪
          setPanelAt({
            top: Math.round(box.bottom + 8 - box.top),                // 相对容器：按钮下方 8px
            right: -shift,                                            // 负数＝从容器往右挪出 shift
            width: width,
            maxHeight: Math.max(220, Math.round(window.innerHeight - box.bottom - 32))
          });
        }
        setOpen(next);
      };
