...
- ✅ **官方计价**：内置 DeepSeek 官方价格表引擎（`pricing.js`），支持：
  ...
  - **节假日与调休**（2026-09-19 官方《API 峰谷时间说明》）：中国法定节假日全天、调休上班的周末一律闲时价；内置 2026 年放假日程与调休上班日
  - 缓存命中 / 缓存写入的差异化计价
  - 推理 token 已含在输出 token 中，**不重复计价**
  - 历史政策时间表：模型改名、降价、下架都按**当时**价格计算，老消息的费用和平台账单对得上
  - 已收录至 **2026-09-19 节假日计费口径**（只加规则、不调价）；此前收录 **2026-09-10 V4.1-Flash 调价**（flash 系列降价、新增模型名 `deepseek-flash`）
...
- 官方调整价格或峰谷规则时，**追加**一个带 `since`（生效时刻，含 `+08:00` 时区偏移）的新条目即可，峰谷规则变化可配合 `weekendOffPeak` / `holidayOffPeak` 等字段表达；
- **节假日表**在同一个文件的 `CN_HOLIDAY_PERIODS`：各节日的放假起止日期 + `makeup`（调休上班日）。国务院一般每年 11 月公布次年安排，届时**追加**下一年度的条目即可，旧条目不要改。`makeup` 里的调休上班日本身是周末、按官方口径仍按空闲时段计费，列出来只为界面能说明"今天要上班但仍是闲时价"，不参与峰谷判定；**漏补了面板会自己提醒**（已进入 11 月而表里还没有次年条目、或今年整年缺失时，「价格设置」页会显示一行提示）；
...
**0.4.4（2026-09-22）**：计费面板改为打开时按可用横向空间定宽（「按钮右边缘 → 对话区左边缘」，上限 440px / 下限 260px），空间仍不足时小幅右移，避免伸进左侧会话区被更高层级盖住；锚点不变，仍挂在「计费」按钮下方。

**0.4.3（2026-09-22）**：面板宽度随窗口自适应（`clamp(320px, 40vw, 440px)`）并加 `overflow-x: hidden`，窄窗口里不再占掉半屏。

**0.4.2（2026-09-22）**：节假日表加「补表自检」——今年整年缺失、或已进入 11 月而缺次年条目时，面板会提示该更新 `CN_HOLIDAY_PERIODS`。

**0.4.1（2026-09-22）**：按官方 2026-09-19《API 峰谷时间说明》补全口径——**调休上班的周末 + 中国法定节假日全天按空闲时段计费**；内置 2026 年放假日程（7 个节日、33 天）与 6 天调休上班日；新增政策条目只加规则不调价；面板标出节假日、并在「高级」里列出全年节假日与调休日。

**0.4.0（2026-09-13）**：价格设置页整理为「当前生效价 / 我的调价记录 / 官方历史价（折叠）」。

**0.3.1（2026-09-13）**：面板区块整理，回归测试随仓库提供（`npm test`）。
