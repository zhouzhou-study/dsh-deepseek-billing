...
const PRICING_ROUTE_PATH = "/api/dsh-deepseek-billing/pricing";
const USAGE_ROUTE_PATH = "/api/dsh-deepseek-billing/usage-by-policy";
/** POST 请求体上限（价格条目很小，64KB 足够）。 */
const MAX_PRICING_BODY_BYTES = 64 * 1024;
...
/** 读 POST 的 JSON 请求体（带上限；永不抛错，一律用返回值表达失败）。 */
function readJsonBody(req) {
  return new Promise((resolve) => {
    ...
    req.on("data", (chunk) => {
      size += chunk.length;
      if (size > MAX_PRICING_BODY_BYTES) {
        resolve({ ok: false, error: "too-large", message: `请求体超过 ${MAX_PRICING_BODY_BYTES} 字节` });
        req.destroy();
        return;
      }
      chunks.push(chunk);
    ...
    req.on("end", () => {
      try {
        ...
        resolve({ ok: true, value: text.trim() === "" ? {} : JSON.parse(text) });
      } catch {
        resolve({ ok: false, error: "bad-json", message: "请求体不是合法 JSON" });
      }
    });
    req.on("error", () => resolve({ ok: false, error: "read-failed", message: "读取请求体失败" }));
  });
}
...
function priceEventInto(record, event) {
  ...
  const unit = priceAtWithUser(model, event.time ?? Date.now(), readSnapshot());
  const sample = costOf(usage, unit);
  // 推理 token **不再额外加价**（2026-09-13 修正的重复计费）：
  //   官方文档：total_tokens = prompt_tokens + completion_tokens，且
  //   `completion_tokens_details.reasoning_tokens` 是「completion tokens 的详细信息」；
  //   DSH 的 outputTokens 就是 completion_tokens ⇒ reasoning 已含在 output 里。
  //   实测 43/43 条带官方 total 的请求满足 total = input+cacheRead+cacheWrite+output，
  //   再加上 reasoning 就对不上。修正前多算一遍会让"想得多"的会话虚高 40% 以上。
  //   reasoningTokens 仍保留在明细里展示（它是 output 的构成，不是额外一份）。
  ...
  record.totalTokens += sample.inputTokens + sample.cacheReadTokens + (usage.cacheWriteTokens ?? 0) + sample.outputTokens;
  ...
  record.requests.push({
    ...
    totalTokens: sample.inputTokens + sample.cacheReadTokens + (usage.cacheWriteTokens ?? 0) + sample.outputTokens,
    cost: roundCost(sample.cost),
    costUsd: roundCost(sample.costUsd),
    priceSource: unit.policy.user === true ? "user" : "official",
    priceMode: unit.mode,
    priceLabel: unit.policy.label,
    priceCny: unit.cny
  });
  return true;
}
...
function apply(ctx) {
  ...
  // ---- pricing table route (GET 读 / POST 写) ----------------------------
  // 用户在界面上维护自己的价格表：GET 返回「当前生效价 + 我的调价记录 + 官方价表」，
  // POST 执行 upsert / delete / config。写盘是原子的；坏数据只会被拒绝或跳过，
  // 绝不会让计费接口本身 500。
  ctx.effect(
    () => ctx.webServer.register({
      kind: "exact",
      ...
      handler: async (req, res) => {
        try {
          ...
          if (method === "GET") {
            sendJson(res, 200, describePricing(readSnapshot({ force: true })));
            return;
          }
          if (method !== "POST") {
            sendJson(res, 405, { ok: false, error: "method-not-allowed", message: `不支持 ${method}` });
            return;
          }
          const body = await readJsonBody(req);
          if (body.ok !== true) {
            sendJson(res, 400, { ok: false, error: body.error, message: body.message });
            return;
          }
          const payload = body.value ?? {};
          const action = typeof payload.action === "string" ? payload.action : "upsert";
          let result;
          if (action === "upsert") result = upsertEntry(payload.entry ?? {});
          else if (action === "delete") result = deleteEntry(typeof payload.id === "string" ? payload.id : "");
          else if (action === "config") result = updateConfig(payload.config ?? {});
          else {
            sendJson(res, 400, { ok: false, error: "unknown-action", message: `未知操作：${action}` });
            return;
          }
          if (result.ok !== true) {
            sendJson(res, 400, {
          ...
          ctx.logger.warn("dsh-deepseek-billing: pricing route failed");
          ctx.logger.warn(error);
          sendJson(res, 500, { ok: false, error: "internal", message: "internal error" });
        }
      }
    }),
    "dsh-deepseek-billing: pricing route"
  );

  // ---- per-policy usage route --------------------------------------------
  // 「每档官方价你实际用了多少」：扫全部会话按当时生效的政策归类。
  // 扫描要几秒，所以走后台 + 缓存：接口立即返回（可能 status=scanning），前端轮询即可。
  ctx.effect(
    () => ctx.webServer.register({
      kind: "exact",
      ...
      handler: async (req, res) => {
        try {
          const persistence = ctx.get("sessionPersistence");
          if (persistence !== void 0) refreshUsage(persistence, readSnapshot());
          const cached = usageCacheState();
          sendJson(res, 200, {
            ok: true,
            status: cached.result === null ? "scanning" : cached.running ? "refreshing" : cached.fresh ? "ready" : "stale",
            ...(cached.result ?? {})
          });
        } catch (error) {
          ctx.logger.warn("dsh-deepseek-billing: usage-by-policy failed");
          ctx.logger.warn(error);
          sendJson(res, 200, { ok: true, status: "scanning" });
        }
      }
    }),
    "dsh-deepseek-billing: usage route"
  );
}
