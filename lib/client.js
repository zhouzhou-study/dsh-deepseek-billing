...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var USAGE_PATH = "/api/dsh-deepseek-billing/usage-by-policy";
    // 价格面板里的「历史用量」：扫本地会话日志，把每次调用按**当时生效的价格政策**
    // 归类，方便看每一档价格实际花了多少。只统计 DSH 自己的会话记录。
    var SHOW_USAGE_HISTORY = true;
    ...
    /** 从"每档用量"结果里取某条政策的用量（按 since 精确匹配）。 */
    function usageBucket(usage, since) {
      if (usage === null || usage === void 0 || !Array.isArray(usage.buckets)) return null;
      for (var i = 0; i < usage.buckets.length; i++) {
        if (usage.buckets[i].since === since) return usage.buckets[i];
      }
      return null;
    }
    ...
    function PricingBody(props) {
      ...
      if (SHOW_USAGE_HISTORY && props.usage !== null && props.usage !== void 0) {
        var usageInfo = props.usage;
        var scanned = usageInfo.status === "ready" || usageInfo.status === "stale";
        rows.push(h("div", { key: "usage-status", style: MUTED },
          scanned
            ? "你的历史用量：共 " + (usageInfo.calls || 0) + " 次调用 · " + fmtMoney(usageInfo.totalCost || 0, "CNY") + (usageInfo.failed > 0 ? "（" + usageInfo.failed + " 个会话读不出来，已跳过）" : "")
            : "正在统计你的历史用量…（后台扫会话，几秒后自动刷新）"
        ));
      }
      if (ui.openHistory) {
        rows.push(h("div", { key: "hist", style: { display: "flex", flexDirection: "column", gap: 6 } }, state.official.map(function (policy, index) {
          ...
          return h("div", { key: key, style: { borderLeft: "2px solid var(--dsw-alias-border-l1)", paddingLeft: 6 } },
            h("div", { style: { display: "flex", alignItems: "center", gap: 6, fontSize: 11, fontWeight: 600, lineHeight: "16px", flexWrap: "wrap" } },
              ...
              SHOW_USAGE_HISTORY ? (function () {
                var bucket = usageBucket(props.usage, policy.since);
                if (bucket === null) return null;
                return h("span", { style: { marginLeft: "auto", fontVariantNumeric: "tabular-nums" } },
                  "你在这档用了 " + bucket.calls + " 次 · " + fmtMoney(bucket.cost, "CNY") + (bucket.user ? "（按我的价算）" : "")
                );
              })() : null
            ),
            h("div", {
    ...
    function PricingSettings() {
      ...
      // 「每档价你实际用了多少」：后台扫会话，未就绪时每 4 秒轮询一次
      useEffect(function () {
        if (!SHOW_USAGE_HISTORY) return void 0;
        var cancelled = false;
        var timer = null;
        var load = async function () {
          try {
            var body = await fetchJson(USAGE_PATH);
            if (cancelled) return;
            setUsage(body);
            if (body && body.status !== "ready" && body.status !== "stale") timer = setTimeout(load, 4000);
          } catch (error) {
