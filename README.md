...
- **节假日表**在同一个文件的 `CN_HOLIDAY_PERIODS`：各节日的放假起止日期 + `makeup`（调休上班日）。国务院一般每年 11 月公布次年安排，届时**追加**下一年度的条目即可，旧条目不要改。`makeup` 里的调休上班日本身是周末、按官方口径仍按空闲时段计费，列出来只为界面能说明"今天要上班但仍是闲时价"，不参与峰谷判定；
